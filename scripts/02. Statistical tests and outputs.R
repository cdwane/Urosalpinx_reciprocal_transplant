...
# Data is loaded directly from the "processed data" folder using read_csv commands, but if script 01. is run first the data will be pre-loaded
...
{
model=glmmTMB(m ~ Treat*Pop*Measure*Sex+ (1|Snail_ID)+ (1|Is_Replacement)+ (1|Egg_tag), REML = TRUE, data=sizedata, family=gaussian(link="identity"))
model2=glmmTMB(m ~ Treat*Pop*Measure*Sex+ (1|Snail_ID)+ (1|Is_Replacement), REML = TRUE, data=sizedata, family=gaussian(link="identity"))
model3=glmmTMB(m ~ Treat*Pop*Measure*Sex+ (1|Snail_ID)+ (1|Egg_tag), REML = TRUE, data=sizedata, family=gaussian(link="identity"))
model4=glmmTMB(m ~ Treat*Pop*Measure*Sex+ (1|Snail_ID), REML = TRUE, data=sizedata, family=gaussian(link="identity"))
}
...
{model=glmmTMB(pgrowth~ Treat*Pop*Measure*Sex+ (1|Snail_ID), REML = TRUE, data=growthdata, family=gaussian(link="identity"),dispformula = ~ Measure*Treat*Pop,control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
  model2=glmmTMB(pgrowth~ Treat*Pop*Measure*Sex+ (1|Snail_ID)+ (1|Is_Replacement)+ (1|Egg_tag), REML = TRUE, data=growthdata, family=gaussian(link="identity"),dispformula = ~ Measure*Treat*Pop,control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
  model3=glmmTMB(pgrowth~ Treat*Pop*Measure*Sex+ (1|Snail_ID)+ (1|Is_Replacement), REML = TRUE, data=growthdata, family=gaussian(link="identity"),dispformula = ~ Measure*Treat*Pop,control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
  model4=glmmTMB(pgrowth~ Treat*Pop*Measure*Sex+ (1|Snail_ID)+ (1|Egg_tag), REML = TRUE, data=growthdata, family=gaussian(link="identity"),dispformula = ~ Measure*Treat*Pop,control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
}
...
# Summary table
tab_df(tidy(Anova(model,type=3)), title = "Tissue weight", show.rownames = FALSE, show.type=FALSE,digits=3)

# Checks to see if other random effects structures were preferred (used in supplemental material)
{
  model=glmmTMB(Tissue_weight~ Treat*Pop*Month*Sex+ (1|Snail_ID)+ (1|Egg_tag)+(1|Is_Replacement), REML = TRUE, data=weightdata, family=Gamma(link="log"),control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
  model2=glmmTMB(Tissue_weight~ Treat*Pop*Month*Sex+ (1|Snail_ID)+ (1|Egg_tag), REML = TRUE, data=weightdata, family=Gamma(link="log"),control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
  model3=glmmTMB(Tissue_weight~ Treat*Pop*Month*Sex+ (1|Snail_ID)+(1|Is_Replacement), REML = TRUE, data=weightdata, family=Gamma(link="log"),control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
  model4=glmmTMB(Tissue_weight~ Treat*Pop*Month*Sex+ (1|Snail_ID), REML = TRUE, data=weightdata, family=Gamma(link="log"),control = glmmTMBControl(optCtrl = list(iter.max = 1000, eval.max = 1000)))
}
...
# Summary table
tab_df(tidy(Anova(modelNC,type=3)), title = "Consumption (Warm regime)", show.rownames = FALSE, show.type=FALSE,digits=3)
...
summary (modelNH)
tab_df(tidy(Anova(modelNH,type=3)), title = "Consumption (Cold regime)", show.rownames = FALSE, show.type=FALSE,digits=3)
...
# Summary table
tab_df(tidy(Anova(model,type=3)), title = "Size at first reproduction", show.rownames = FALSE, show.type=FALSE,digits=3)
...
# Summary table
tab_df(tidy(Anova(model,type=3)), title = "Total reproductive output", show.rownames = FALSE, show.type=FALSE,digits=3)
...
cold <- matrix(c(8, 10,
               ...
               nrow = 2,
               byrow = TRUE)
colnames(cold) <- c("Reproduced", "Not_reproduced")
rownames(cold) <- c("Low_lat", "High_lat")
